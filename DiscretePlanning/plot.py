...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 28 11:27:26 2021
...
def plot_map(environment, component=None, values=None, ax=None, min_val=None, max_val=None, 
    ...
    values = np.ones(environment.n_locations) if values is None else values
    ...
    # Get components: if not specified, use all except base (because that will just be everywhere)
    component = [k for k in environment.components.keys() if k != 'base'] if component is None else [component]
    # After drawing all locations, add component patches
    for location in environment.locations:
        # Create colour for each component (do x2 to get more different cols)
        comp_cm = cm.get_cmap('tab10', len(component))
...
def plot_actions(locations, field='probability', ax=None, min_val=None, max_val=None, radius=None,
                 num_cols=100, action_cm='viridis', components=None, do_plot_locations=True):
    """
    ...
    location_cm = {comp: cm.get_cmap('tab10', len(components)*3)(c_i) for c_i, comp in enumerate(components)}
    # Calculate radius of location circles based on how many nodes there are
    radius = 1 * (0.01 + 1 / (10 * np.sqrt(len(locations)))) if radius is None else radius
    # Initialise empty axis
    ax = initialise_axes(ax)
    # Create empty list of location patches and action patches
    location_patches, action_patches = [], []
    ...
    for i, location in enumerate(locations):
        ...
        loc_col = location_cm[loc_comp[0]] if len(loc_comp) > 0 else [0, 0, 0]
        # Create circle patch for location
        location_patches.append(plt.Circle((location['x'], location['y']), radius, color=loc_col))
    ...
    # Add patches to axes
    for patch in ((location_patches + action_patches) if do_plot_locations else action_patches):
        ax.add_patch(patch)
    # Return axes for further use
    return ax


def plot_grid(environment, locations=None, ax=None, do_plot_actions=True,
              values=None, min_val=None, max_val=None, val_cm='viridis'):
    """ Restricted but neat plotting of square grid graphs """
    blue, green = cm.get_cmap('tab10',10)([0, 2])
    ...
    if locations is None:
        locations = environment.get_policy(in_place=False)
    else:
    ...
    # Colour map for locations: walls are black, locations white, reward green
    location_cm = {key: (green if val['type'] == 'shiny' else [0, 0, 0])
                   for key, val in environment.components.items()
                   if val['type'] in ['shiny', 'wall']}
        ...
        if len(loc_comp) == 0:
            for a, action in enumerate(location['actions']):
                ...
                if action['probability'] > 0:
                    ...
                    # Create an action patch for each possible transition for this action
                    for loc_to in locations_to:
                        action_patches.append(action_patch(location, loc_to, radius, blue, gap=False))
    # Add patches to axes
...
def plot_walk(environment, positions, max_steps=None, n_steps=1, ax=None, pred_correct=None, cmap='Reds', 
    ...
    for step_i in range(1, max_steps, n_steps):
        ...
        new_loc = new_loc + jitter * (-radius + 2 * radius * np.random.rand(*new_loc.shape))
        ...
        ax.plot([prev_loc[0], new_loc[0]], [prev_loc[1], new_loc[1]], color=cmap(step_i / max_steps), linewidth=width)
